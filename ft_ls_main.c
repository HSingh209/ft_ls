...
/*This function will create a list of the nodes that are a DIR inside
...
t_dir_info 	*ft_make_nodes(char **av, t_node *flags)
{
	...
	if (av[i])	//While there are argv passed in, create a list to work with
	{
		...
		{
			path = ft_setPath_fs(av[i], "./");
			ft_push(&linked_lst, ft_new_chdir((ft_strdup(av[i])), path, flags, &temp));	//removed ft_strdup(av[i])
			i++;
		}
	}
	else
